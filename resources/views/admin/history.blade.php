@php
    ...
    $historyData = collect([
        ...
        [
            ...
            'avatar' => 'BS',
            'action' => 'Usage',
            'item_name' => 'Mousepad XL',
            'category' => 'Accessories',
            'quantity' => -1,
            'note' => 'Penggantian untuk Divisi HR',
            'created_at' => '2026-01-27 10:15:00',
        ],
        [
            'user' => 'Siti Aminah',
            'role' => 'Admin Gudang',
            'avatar' => 'SA',
            'action' => 'Restock',
            'item_name' => 'Paper A4 Rim',
            'category' => 'Stationery',
            'quantity' => 50,
            'note' => 'Stok bulanan rutin',
            'created_at' => '2026-01-26 16:45:00',
        ],
        [
            'user' => 'Joko Anwar',
            'role' => 'Staff IT',
            ...
            'category' => 'Accessories',
            'quantity' => -2,
            'note' => 'Peminjaman untuk Meeting Room A',
            'created_at' => '2026-01-26 14:20:00',
        ],
        [
            'user' => 'System',
            'role' => 'System',
            ...
            'category' => 'Electronics',
            'quantity' => -1,
            'note' => 'Barang rusak (Broken Stock)',
            'created_at' => '2026-01-25 11:00:00',
        ],
    ]);

    // Statistik Harian
    $transactionsToday = 2; // Dummy
    $itemsInToday = 5;
    $itemsOutToday = 1;
@endphp
    ...
    <div class="w-full max-w-7xl mx-auto px-6 py-6 space-y-8 font-sans">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-slate-800 mt-1">{{ $transactionsToday }} <span class="text-sm font-normal text-slate-400">Actions</span></h3>
                </div>
            ...
            <div class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-emerald-600 mt-1">+{{ $itemsInToday }}</h3>
                </div>
            ...
            <div class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-rose-600 mt-1">-{{ $itemsOutToday }}</h3>
                </div>
        ...
        <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div class="px-6 py-5 border-b border-slate-100 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 bg-slate-50/50">
                ...
                <div class="flex flex-col sm:flex-row gap-3 w-full xl:w-auto">
                    <div class="flex items-center gap-2 bg-white px-3 py-2 border border-slate-200 rounded-xl shadow-sm">
                        <svg xmlns="http://www.w3.org/2000/svg" class="size-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                        <input type="date" class="text-sm border-none p-0 text-slate-600 focus:ring-0 w-28" value="{{ date('Y-m-01') }}">
                        <span class="text-slate-300">-</span>
                        <input type="date" class="text-sm border-none p-0 text-slate-600 focus:ring-0 w-28" value="{{ date('Y-m-d') }}">
                    </div>
                ...
                <table class="min-w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @foreach ($historyData as $log)
                            <tr class="history-row hover:bg-slate-50 transition-colors" data-action="{{ $log['quantity'] > 0 ? 'in' : 'out' }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="text-sm font-medium text-slate-700">{{ $log['user'] }}</p>
                                            <p class="text-[11px] text-slate-400">{{ $log['role'] }}</p>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-center whitespace-nowrap">
                                    @php
                                        ...
                                        if ($log['action'] == 'Restock') {
                                            ...
                                            $actionIcon = '<svg class="size-3 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg>';
                                        } elseif ($log['action'] == 'Usage') {
                                            $actionClass = 'bg-rose-50 text-rose-700 border-rose-100';
                                            $actionIcon = '<svg class="size-3 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M20 12H4"/></svg>';
                                        } else {
                                            $actionClass = 'bg-blue-50 text-blue-700 border-blue-100'; // For Adjustment
                                            $actionIcon = '<svg class="size-3 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>';
                                        }
                                    @endphp
                                ...
                                <td class="px-6 py-4">
                                    <p class="text-sm font-semibold text-slate-700">{{ $log['item_name'] }}</p>
                                    <p class="text-xs text-slate-500">{{ $log['category'] }}</p>
                                </td>
            ...
            <div class="px-6 py-4 border-t border-slate-100 flex items-center justify-between bg-slate-50/50">
                <span class="text-xs text-slate-500">Showing last 5 activities</span>
    ...
    <script>
        function filterHistory(type) {
            ...
            Object.keys(tabs).forEach(key => {
                if (key === type) {
                    tabs[key].className = "px-4 py-1.5 rounded-lg text-sm font-semibold bg-white text-slate-800 shadow-sm transition-all";
                } else {
                    tabs[key].className = "px-4 py-1.5 rounded-lg text-sm font-medium text-slate-500 hover:text-slate-700 transition-all";
                }
            });

            // 2. Filter Rows
            rows.forEach(row => {
                const actionType = row.getAttribute('data-action'); // 'in' or 'out'
        ...
        function searchHistory() {
            ...
            rows.forEach(row => {
                // Search in User (col 2), Item (col 4), Note (col 6)
