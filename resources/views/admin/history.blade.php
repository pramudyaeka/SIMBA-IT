    ...
    <div class="w-full max-w-7xl mx-auto px-6 py-6 space-y-8 font-jakarta">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                ...
                <p class="text-slate-500 text-sm mt-1">Track inventory movements and system activities.</p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-slate-800 mt-1">
                        {{ $transactionsToday }} <span class="text-sm font-normal text-slate-400">Actions</span>
                    </h3>
                </div>
            ...
            <div class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-rose-600 mt-1">{{ $itemsOutToday < 0 ? $itemsOutToday : '-' . $itemsOutToday }}</h3>
                </div>
                ...
                <table class="min-w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @foreach ($historyData as $log)
                            {{-- LOGIKA DATA ACTION: Jika tidak ada item_id, anggap sebagai 'system', jika ada qty>0 'in', else 'out' --}}
                            @php
                                $dataAction = 'system';
                                if($log->item_id) {
                            ...
                            <tr class="history-row hover:bg-slate-50 transition-colors" data-action="{{ $dataAction }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-xs text-slate-400">{{ \Carbon\Carbon::parse($log->created_at)->format('H:i') }} WIB</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-center whitespace-nowrap">
                                    @php
                                        ...
                                        } elseif (in_array($log->action, ['Create User', 'Update User', 'Delete User', 'Create'])) {
                                ...
                                <td class="px-6 py-4">
                                    @if($log->item_id)
                                        {{-- Jika log BARANG --}}
                                        <p class="text-sm font-semibold text-slate-700">{{ $log->item->item_name ?? 'Deleted Item' }}</p>
